...
impl Config {
    fn addr(&self) -> SocketAddr {
        let addr = self.address.parse().expect("Unable to parse socket address");
...
impl Default for Config {
    fn default() -> Self {
        Self { address: DEFAULT_ADDRESS.to_string(), port: DEFAULT_PORT, ath_file_path: PathBuf::from(DEFAULT_ATH_FILE_PATH) }
    }
}
