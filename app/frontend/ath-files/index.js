...
value: `
#********************************************************************
# Here we prove that any binary relation R that is irreflexive and 
# transitive must also be asymmetric.
#********************************************************************
module Asymmetry {
domain D
declare <: [D D] -> Boolean
...
`
  ...
  "/contrapositive.ath": {
      fname: "contrapositive.ath",
      value: 
      `# Here we show that if P(x) implies Q(x) for all x, then ~Q(a) implies ~P(a):
...
(!prove ((forall x . P x ==> Q x) ==> ~ Q a ==> ~ P a) (ab))`
  ...
  "/list-concat-assoc.ath": {
      fname: "list-concat-assoc.ath",
      value:
      `# Using ATPs, we show that list concatenation is associative. 

# First, introduce a function symbol for (polymorphic) list concatenation:
...
# Let's define the concatenation operation using recursion:
...
# Let's try to falsify this conjecture automatically:

(falsify ++-assoc 10)

# We were not able to falsify it in a small scope, 
# so let's try to prove it automatically: 

(!list-induction ++-assoc)`
  },
  "/quantifiers.ath": {
      fname: "quantifiers.ath",
      value:
...
# The function reflect takes a tree and recursively
# reflects it along the y axis: 

declare reflect: (S) [(BTree S)] -> (BTree S)

# Introduce a few variables for convenience:
...
let {[ih1 ih2 ] := [(reflect reflect left = left)
  ...
  "/propositional-logic.ath": {
      fname: "propositional-logic.ath",
      value:
...
(!by-contradiction (~ A)
  ...
  "/propositional-logic-2.ath": {
      fname: "propositional-logic-2.ath",
      value:
...
assume F
  ...
  "/automated-theorem-proving.ath": {
    fname: "automated-theorem-proving.ath",
    value: 
...
# Let's derive this automatically for n = 8 using Vampire:

(!derive-from (pigeonhole 8) [] |{'atp := 'vampire, 'max-time := 100}|)`
  },
  "/smt-example.ath" : {
...
# Options for the SMT solving. Set the solver to cvc4
# and the results to be stored in the above hash table.

define options := |{'solver := 'cvc, 'results := ht }|

# Ask if there is an integer x that is greater than 1 and less than 3: 

(SMT.smt-solve (x > 1 & x < 3) options)

# The procedure returns one of the following: 'Satisfiable,
# 'Unsatisfiable, or 'Unknown. If the result is 'Satisfiable, then
# a satisfying interpretation is stored in the hash table.
